...
export async function createUser(username, passwordHash) {
  try {
    const [result] = await db.execute('INSERT INTO users (username, password_hash, role) VALUES (?, ?, "user")', [username, passwordHash]);
...
export async function getUserById(id) {
  try {
    const [rows] = await db.execute('SELECT id, username, bio, favorite_genre, theme, is_pro, spotify_id, role, created_at FROM users WHERE id = ?', [id]);
    if (!rows[0]) return null;
    return {
      ...rows[0],
