...
export async function saveArtist(artistData) {
  const { id, name, bio, genres, profile_image, persona_prompt, custom_nudge_message, achievements, tours, songs, featured_opinions, knowledge_injection, social_links } = artistData;
  try {
    let artistId = id;
    if (artistId) {
      await db.execute(
        'UPDATE artists SET name=?, bio=?, specialized_genres=?, profile_image=?, persona_prompt=?, custom_nudge_message=?, social_links=? WHERE id=?',
        [name, bio, JSON.stringify(genres), profile_image, persona_prompt, custom_nudge_message, JSON.stringify(social_links || []), artistId]
      );
    } else {
      const [res] = await db.execute(
        'INSERT INTO artists (name, bio, specialized_genres, profile_image, persona_prompt, custom_nudge_message, social_links) VALUES (?, ?, ?, ?, ?, ?, ?)',
        [name, bio, JSON.stringify(genres), profile_image, persona_prompt, custom_nudge_message, JSON.stringify(social_links || [])]
      );
    ...
    for (const item of knowledgeMap) {
      // We use the category and a title pattern to identify these items
...
export async function getAllArtists() {
  try {
    const [rows] = await db.query('SELECT id, name, specialized_genres, profile_image, persona_prompt, created_at FROM artists WHERE is_deleted = 0 ORDER BY name ASC');
    return rows;
  } catch (err) {
